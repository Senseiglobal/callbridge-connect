...
The website form, server-only database queue and local operator's authenticated queue connection were verified on September 13. On September 14, one fresh consented request completed an operator-approved SDK call, a real two-way conversation, and an operator-triggered result refresh that saved the genuine brief to the same Aura request. See [the pilot evidence](VERIFIED-AURA-CALLBACK.md). No plan-specific entitlement, pricing promise, account change, booked human follow-up or automatic creative-project writeback is claimed. Verify product details against Aura's current information before answering customer questions; otherwise defer to a human.
