...
## 1. Put the demo on the internet

Open [Deploy CallBridge](https://render.com/deploy?repo=https://github.com/Senseiglobal/callbridge-connect).

1. Sign in or create a Render account. If asked to connect GitHub, limit access to the CallBridge repository where possible.
2. Review the service named `callbridge-connect`. The prepared configuration selects the free plan and does not need your CALL-E key.
3. Approve the deployment. If it asks you to buy anything or change the plan, pause and tell Codex what you see.
4. Wait for **Live**, then open the web address Render gives you. Copy that address back to Codex for verification.
5. Choose **New check-in**, then **Create sample check-in**, then **Preview callback**. These are fictional examples, not real calls.
...
Optional: use [our observed feedback draft](CALLE-FEEDBACK.md) for the CALL-E feedback survey. It is separate from the main submission.

## Things you do NOT need for this CALL-E contest

Revenue, a company registration number, a P&L and Google Cloud usage came from the other 90-day competition. Do not let them delay this submission. Do not invent customer testimonials, revenue or impact numbers.
