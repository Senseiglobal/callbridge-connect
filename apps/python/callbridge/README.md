...
For the signed-in Aura pilot, `STORAGE_BACKEND=aura` uses a narrow authenticated queue API backed by Aura's existing database. It requires a separate operator access code even on localhost and rejects manual/sample creation: new requests must come from Aura's authenticated consent form. See [pilot configuration](../../../docs/aura-pilot-setup.md). One operator-approved SDK call and its genuine result saved to the same Aura request were verified on September 14, 2026; see [the evidence boundary](../../../docs/VERIFIED-AURA-CALLBACK.md). Public demo mode always uses its isolated sample store.
