...
For the signed-in Aura pilot, `STORAGE_BACKEND=aura` uses a narrow authenticated queue API backed by Aura's existing database. It requires a separate operator access code even on localhost and rejects manual/sample creation: new requests must come from Aura's authenticated consent form. See [pilot configuration](../../../docs/aura-pilot-setup.md). The schema is installed; the deployed integration and real call still require acceptance checks. Public demo mode always uses its isolated sample store.
